...
function waitForJQuery() {
    ...
    // Try multiple ways to get jQuery
    const $ = unsafeWindow.$ || unsafeWindow.jQuery || window.$ || window.jQuery;

    if ($) {
        console.log(`SpookyX: Found jQuery after ${attempts} attempts, version:`, $.fn.jquery);
        initScript($);
        return;
    }

    if (attempts >= maxAttempts) {
        console.error('SpookyX: Could not find jQuery after', maxAttempts, 'attempts');
        return;
    }
...
function initScript($) {
    ...
    // Load plugins inline to avoid more loading issues
    loadPluginsInline($);

    // Wait a bit more for everything to settle
    setTimeout(() => startMainScript($), 500);
}
function loadPluginsInline($) {
    // jQuery Selection Plugin - clean version
    if (!$.fn.selection) {
        (function($, window, document) {
            var getCaretInfo = function(element) {
                var info = { text: "", start: 0, end: 0 };
                if (!element.value) return info;

                try {
                    if (window.getSelection) {
                        info.start = element.selectionStart;
                        info.end = element.selectionEnd;
                        info.text = element.value.slice(info.start, info.end);
                    } else if (document.selection) {
                        element.focus();
                        var range = document.selection.createRange();
                        var bodyRange = document.body.createTextRange();
                        info.text = range.text;

                        try {
                            bodyRange.moveToElementText(element);
                            bodyRange.setEndPoint("StartToStart", range);
                        } catch (e) {
                            bodyRange = element.createTextRange();
                            bodyRange.setEndPoint("StartToStart", range);
                        }

                        info.start = element.value.length - bodyRange.text.length;
                        info.end = info.start + range.text.length;
                    }
                } catch (e) { }
            ...
            var methods = {
                getPos: function(element) {
                    var info = getCaretInfo(element);
                    return { start: info.start, end: info.end };
                },

                setPos: function(element, pos, caret) {
                    caret = caret || "keep";
                    if (caret === false) caret = "end";
                    ...
                    try {
                        if (element.createTextRange) {
                            ...
                            if (window.navigator.userAgent.toLowerCase().indexOf("msie") >= 0) {
                                pos.start = element.value.substr(0, pos.start).replace(/\r/g, "").length;
                                pos.end = element.value.substr(0, pos.end).replace(/\r/g, "").length;
                            }
                            range.collapse(true);
                            ...
                            element.setSelectionRange(pos.start, pos.end);
                        }
                    } catch (e) { }
                },
                ...
                replace: function(element, text, caret) {
                    ...
                    var newPos = { start: info.start, end: info.start + text.length };

                    element.value = value.substr(0, info.start) + text + value.substr(info.end);
                    $(element).scrollTop(scrollTop);
                    this.setPos(element, newPos, caret);
                },
                ...
                insertBefore: function(element, text, caret) {
                    ...
                    var value = element.value;
                    var scrollTop = $(element).scrollTop();
                    var newPos = { start: info.start + text.length, end: info.end + text.length };

                    element.value = value.substr(0, info.start) + text + value.substr(info.start);
                    $(element).scrollTop(scrollTop);
                    this.setPos(element, newPos, caret);
                },
                ...
                insertAfter: function(element, text, caret) {
                    ...
                    var value = element.value;
                    var scrollTop = $(element).scrollTop();
                    var newPos = { start: info.start, end: info.end };

                    element.value = value.substr(0, info.end) + text + value.substr(info.end);
                    $(element).scrollTop(scrollTop);
                    this.setPos(element, newPos, caret);
                }
            };
            ...
            $.extend({
                selection: function(type) {
                    ...
                    try {
                        if (window.getSelection) {
                            if (isText) return window.getSelection().toString();
            ...
            $.fn.extend({
                selection: function(method, options) {
                    ...
                    switch (method) {
                        case "getPos":
                            return methods.getPos(this[0]);
                        case "setPos":
                            return this.each(function() {
                                methods.setPos(this, options);
                            });
                        case "replace":
                            return this.each(function() {
                                methods.replace(this, options.text, options.caret);
                            });
                        case "insert":
                            return this.each(function() {
                                if (options.mode === "before") {
                                    methods.insertBefore(this, options.text, options.caret);
                                } else {
                                    methods.insertAfter(this, options.text, options.caret);
                                }
                            });
                        case "get":
                        default:
                            return methods.getText(this[0]);
                    }
    ...
    // Basic mousewheel implementation if needed
    if (!$.fn.mousewheel) {
        $.fn.mousewheel = function(fn) {
            return this.on('wheel mousewheel DOMMouseScroll', function(e) {
                var delta = e.originalEvent.deltaY || e.originalEvent.wheelDelta || e.originalEvent.detail;
                fn.call(this, e, delta > 0 ? -1 : 1);
            });
        };
    }
}
...
function startMainScript($) {
    console.log('SpookyX: Main script starting with jQuery', $.fn.jquery);
    ...
    class PostGraph {
        ...
        getReplyChainFromPost(startPostId, includeOP = true, maxPosts = Infinity, around = false) {
            ...
            if (around) {
                ...
                this.collectFullContext(startPostId, toProcess, opId);
            }
            ...
            const calculateLevel = (postId) => {
                ...
                // Get actual quoted parents from post content
                ...
                // If quotes OP directly, always level 1
                if (quotedParents.includes(opId)) {
                    levelMap.set(postId, 1);
                    this.log(`🎯 ${postId} -> L1 (quotes OP directly)`);
                    return 1;
                }

                // If no quotes, check if it's a direct child of OP in graph
                if (quotedParents.length === 0) {
                    if (node.parents && node.parents.has(opId)) {
                        levelMap.set(postId, 1);
                        this.log(`🎯 ${postId} -> L1 (direct graph child of OP)`);
                        return 1;
                    } else {
                        levelMap.set(postId, 1);
                        this.log(`🎯 ${postId} -> L1 (no quotes, assumed L1)`);
                        return 1;
                    }
                }

                // Calculate based on quoted parents
                const parentLevels = quotedParents
                    .map(pid => levelMap.get(pid))
                ...
                // Use minimum parent level + 1 for consistent threading
                const level = Math.min(...parentLevels) + 1;
                levelMap.set(postId, level);
                this.log(`🎯 ${postId} -> L${level} (min parent level ${Math.min(...parentLevels)} + 1)`);
                return level;
            };
            ...
            // Deduplicate posts by ID while preserving order
            const uniqueResults = Array.from(new Map(result.map(post => [post.id, post])).values());
    ...
    async function displayReplyChain(replyChain, $container) {
        ...
        for (const {
            ...
            try {
                ...
                const $levelHeader =
                    $(`<div class="level-${level}" style="margin: 5px 0 2px 0; font-size: 12px; font-weight: bold; color: ${isOP ? "#773311" : "#555"
                        };">
