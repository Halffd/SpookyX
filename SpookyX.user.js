...
function startMainScript() {
    ...
    class ConversationTree {
        ...
        buildFromPostGraph(startPostId, mode = 'around', includeOp = true) {
            ...
            const relevantPosts = this.collectRelevantPosts(startPostId, mode, includeOp);
            ...
            relevantPosts.forEach(postId => {
                ...
                if (node) {
                    ...
                    reply.quotedParents = extractParentIds(commentText || "")
                        .filter(pid => relevantPosts.has(pid));
        ...
        attachToParent(reply, processed, processing) {
            ...
            if (bestParent) {
                ...
                this.log(`✅ Multi-quote ${reply.id} attached to latest parent ${bestParent.id} (score: ${bestScore})`);
                return true;
            }
            ...
            while (changed && pass < maxPasses) {
                ...
                for (const [postId, reply] of this.allReplies.entries()) {
                    ...
                    if (success) {
                        ...
                        changed = true;
                        this.log(`✅ Attached ${postId} to ${reply.parent?.id} at level ${reply.level}`);
                    }
                }
            }
        ...
        collectRelevantPosts(startPostId, mode, includeOp = true) {
            ...
            if (mode === 'around') {
                ...
                if (startNode) {
                    ...
                    quotedParents.forEach(pid => {
                        if (pid !== opId) { // Skip OP replies here
                ...
                while (queue.length > 0 && collected < maxPosts) {
                    ...
                    const quotedParents = extractParentIds(commentText || "");
                    quotedParents.forEach(pid => {
                        if (this.postGraph.nodes.has(pid) && !visited.has(pid) && collected < maxPosts) {
                            queue.push(pid);
                        }
                    });
        ...
        getPathFromOpToPost(opId, targetPostId) {
            ...
            while (currentId && currentId !== opId && !visited.has(currentId)) {
                ...
                const validParents = quotedParents.filter(pid => this.postGraph.nodes.has(pid));
        ...
        getOrganizedThreads() {
            ...
            const sortedRoots = Array.from(this.roots.values())
                .sort((a, b) => parseInt(a.id) - parseInt(b.id));
        ...
        printTree() {
            ...
            const printNode = (reply, indent = '') => {
                const marker = reply.isOP ? '👑' : reply.children.length > 0 ? '💬' : '💭';
                const quotes = reply.quotedParents.length > 0 ? ` (→${reply.quotedParents.join(',')})` : '';
    ...
    function createPostElement(data) {
        ...
        if (data.media && data.media.media_link) {
          ...
          <span class="post_file_metadata">${filesize}${dimensions ? ", " + dimensions : ""
                }</span>
      `;
      ...
      <ul class="dropdown-menu" role="menu">
          ...
          <li><a href="#" data-post="${data.doc_id || ""}" data-post-id="${data.num || data.no
  ...
  `;
        ...
        postControls.innerHTML = `
          ...
          <a href="#" class="btnr parent" data-post="${data.doc_id || ""
    ...
    const fetchReplies = async (link) => {
        ...
        if (!thread || (thread.error && thread.error.length > 0)) throw new Error("Thread error")
    ...
    async function displayReplyChain(replyChain, $container) {
        ...
        for (const item of replyChain) {
            try {
                ...
                const replyCount = hasChildren !== undefined
                    ? (hasChildren ? '??' : '0') // Use hasChildren if available
                    : (replies?.length || 0); // Fallback to replies array

                const parentList = quotedParents && quotedParents.length > 0
                    ? quotedParents
                ...
                const $levelHeader = $(`<div class="level-${level}" style="margin: 5px 0 2px 0; font-size: 12px; font-weight: bold; color: ${
                    isOP ? "#773311" : "#555"
                };">
                    ${isOP ? "OP" : `L${level}`} (${replyCount}↓${parentList.length}↑)
                    ${parentList.length > 0 ? ` ← [${parentList.join(", ")}]` : ""}
                    ${parentCount !== parentList.length ? ` (graph: ${parentCount} parents)` : ""}
                </div>`);
                ...
                $postContainer.css({
                    ...
                    "border-left": level > 0 && !isOP ? `${Math.min(level, 4)}px solid #ccc` : "none",
        ...
        try {
            ...
            if (country.length != 0) {
                country.each(function() {
                    ...
                    title = title.replace('Click here to search for posts with this flag', '')
                    // Get continent color
    ...
    function toggleAutoExpand() {
        ...
        if ($indicator.length === 0) {
            $("body").append(
                `<div id="auto-expand-indicator" style="position: fixed; top: 10px; right: 10px; background: ${settings.UserSettings.autoExpand.value ? "#4CAF50" : "#f44336"
                }; color: white; padding: 5px 10px; border-radius: 3px; z-index: 9999; font-size: 12px;">Auto-expand: ${settings.UserSettings.autoExpand.value ? "ON" : "OFF"
                }</div>`
            );
        } else {
            $indicator
    ...
    if (settings.UserSettings.inlineImages.suboptions.customSize.value) {
        imageWidthOP =
            settings.UserSettings.inlineImages.suboptions.customSize.suboptions.widthOP
                .value;
        imageHeightOP =
            settings.UserSettings.inlineImages.suboptions.customSize.suboptions.heightOP
                .value;
        imageWidth =
                    ...
                    for (i = 0; i < scale; i++) {
                        for (var j = 0; j < scale; j++) {
                            for (
                                ...
                                for (
                                    var y = 0, height = faviconCanvas.height, heights = height * 4;
                                    y < heights;
                                    y += 4
                                ) {
                                    var start = x + y * width;
        ...
        posts.each(function(i, post) {
            ...
            $imageBoxes.each(function(j, currentImage) {
                ...
                $currentImage.find(">a").each(function(k, imgLink) {
                    ...
                    if (
                        settings.UserSettings.inlineImages.suboptions.processSpoiler.value &&
                ...
                (isOP
                    ...
                    if (mediaType == "image" || mediaType == "video") {
                        ...
                        var filename =
                            ...
                            mediaLink +
                            '">' +
                            mediaLink.match(/[^\/]*/g)[mediaLink.match(/[^\/]*/g).length - 2] +
                            "</a></div>";
                        var spoiler = "";
                        var $elem = $(
                        ...
                        if (
                            settings.UserSettings.embedImages.suboptions.titleYoutubeLinks.value
                        ) {
                        ...
                        if (imgurLinkFragments[3] == "") {
                            ...
                            if (
                                settings.UserSettings.embedGalleries.suboptions.showDetails.value
                            ) {
                                $currentArticle
                                    .find(".post_wrapper")
                                    .prepend(
    ...
    function postCounter() {
        if (!Page.is("other,statistics")) {
            ...
            (locationHeader ? $threadStats : $rules_box).html(
                ...
                (limits
                    ? "/" +
                    settings.UserSettings.postCounter.suboptions.limits.suboptions.posts
                        .value
                    : "") +
                (locationHeader ? "" : "<br>") +
                " Images: " +
    ...
    function parseMascotImageValue() {
        ...
        if (
            ...
            if (
                settings.UserSettings.mascot.suboptions.mascotImage.value === undefined ||
        ...
        if (!$("#file_image").length) {
            ...
            if (!$("#middleReplySubmit").length) {
                ...
                $middleReplySubmit.on("click", function() {
                    ...
                    if (
                        settings.UserSettings.autoHost.value.value !== "Don't reupload links"
                    ) {
                        var fourChanOnly =
                ...
                if (
                    ...
                    if (request.responseText !== "") {
                        try {
                            ...
                            response = {
                                error: "SpookyX encountered an error when parsing the response.",
                            };
                            console.log("The following is the request responseText:");
                            console.log(request.responseText);
                            alert(
        ...
        function clickHandler(e, called = false) {
            if (
                ...
                if ($video.hasClass("fullVideo")) {
                    if (
                        !settings.UserSettings.inlineImages.suboptions.inlineVideos.suboptions
                            .firefoxCompatibility.value ||
                ...
                if (settings.UserSettings.hidePosts.suboptions.recursiveHiding.value) {
                    ...
                    if (button.attributes["data-function"].value === "showPost") {
                        recursiveToggle(
                            $("article.doc_id_" + button.attributes["data-doc-id"].value).attr(
                                "id"
                            ),
                            "show"
                        );
