  ...
  if (jQuery && !jQuery.fn.error) {
    ...
    jQuery.fn.load = function (handler) {
  ...
  function startMainScript() {
            ...
            restrict: {
              ...
              suboptions: {
                lines: {
                  name: "Line count",
                  description:
                    "Number of lines the notification is restricted to",
                  type: "number",
                  value: 5,
                },
                characters: {
        ...
        adjustReplybox: {
          ...
          suboptions: {
            width: {
              name: "Width",
              description:
                "Specify the default width of the reply field in pixels",
              type: "number",
              value: 600,
            },
            hideQROptions: {
        ...
        mascot: {
          ...
          suboptions: {
            ...
            opacity: {
              name: "Opacity",
              description:
                "Specify the opacity of the mascot, ranges from 0 to 1",
              type: "number",
              value: 1,
            },
            clickthrough: {
            ...
            y: {
              ...
              description:
                "Specify vertical displacement of the mascot in pixels",
              type: "number",
              value: 0,
            },
            mute: {
              name: "Mute videos",
              description:
                "If using a video for a mascot the sound will be muted",
              type: "checkbox",
              value: true,
            },
          },
        ...
        headerBar: {
          ...
          suboptions: {
            ...
            shortcut: {
              name: "Hide shortcut",
              description:
                "Pressing H will toggle the visiblity of the headerbar",
              type: "checkbox",
              value: true,
            },
          },
    ...
    class Reply {
      ...
      getAllDescendants() {
        ...
        const traverse = (reply) => {
          result.push(reply);
          const sortedChildren = reply.children
            .slice()
      ...
      findById(postId) {
        if (this.id === postId) return this;

        for (const child of this.children) {
          const found = child.findById(postId);
      ...
      getThread() {
        return this.getAllDescendants().map((reply) => ({
          id: reply.id,
          level: reply.level,
          data: reply.data,
          quotedParents: reply.quotedParents,
          isOP: reply.isOP,
          hasChildren: reply.children.length > 0,
          parentId: reply.parent?.id || null,
        }));
      }
    }

    class ConversationTree {
      constructor(postGraph) {
        this.postGraph = postGraph;
        this.roots = new Map();
        this.allReplies = new Map();
        this.DEBUG = true;
      }
      ...
      buildFromPostGraph(startPostId, mode = "around", includeOp = true) {
        this.log(`🏗️ Building tree from PostGraph for ${startPostId} (${mode})`);

        this.roots.clear();
        this.allReplies.clear();

        const relevantPosts = this.collectRelevantPosts(
          startPostId,
          mode,
          includeOp
        );
        ...
        relevantPosts.forEach((postId) => {
          const node = this.postGraph.nodes.get(postId);
          if (node) {
            const reply = new Reply(postId, node.data, 0);
            const commentText = node.data?.comment || node.data?.com || "";
            reply.quotedParents = extractParentIds(commentText || "").filter(
              (pid) => relevantPosts.has(pid)
            );
            reply.isOP = this.postGraph.findOP(postId) === postId;
            this.allReplies.set(postId, reply);
          }
        });

        // SIMPLIFIED tree building - just use quote relationships directly
        this.buildSimpleTree();

        return this;
      }
      attachToParent(reply, processed, processing) {
        processing.add(reply.id);

        let bestParent = null;
        let bestScore = -1;

        for (const quotedId of reply.quotedParents) {
          if (!this.allReplies.has(quotedId)) continue;
          if (!processed.has(quotedId)) {
            processing.delete(reply.id);
            return false;
          }
          if (processing.has(quotedId)) {
            this.log(`🔄 Cycle detected: ${reply.id} -> ${quotedId}`);
            continue;
          }

          const parentReply = this.allReplies.get(quotedId);

          // FIXED: Score parents by chronological order AND tree depth
          // Prefer the LATEST quoted post that's still before this reply
          const chronologicalScore = parseInt(quotedId);
          const depthPenalty = parentReply.level * 100; // Small penalty for deeper nesting
          const score = chronologicalScore - depthPenalty;
        ...
        if (bestParent) {
          ...
          this.log(
            `✅ Multi-quote ${reply.id} attached to latest parent ${bestParent.id} (score: ${bestScore})`
          );
          return true;
        }
      ...
      buildSimpleTree() {
        this.log(`🌳 Building simple tree structure`);

        const processed = new Set(); // MISSING - need to track processed posts

        // Step 1: Find all root posts
        for (const [postId, reply] of this.allReplies.entries()) {
          if (reply.isOP || reply.quotedParents.length === 0) {
            this.roots.set(postId, reply);
            reply.level = 0;
            processed.add(postId); // MARK AS PROCESSED
            this.log(`🎯 Root found: ${postId} (isOP: ${reply.isOP})`);
          }
        }
        ...
        while (changed && pass < maxPasses) {
          pass++;
          changed = false;
          const processing = new Set(); // MISSING - need for cycle detection
          this.log(`\n=== PASS ${pass} ===`);

          for (const [postId, reply] of this.allReplies.entries()) {
            if (processed.has(postId)) continue; // Skip already processed

            const success = this.attachToParent(reply, processed, processing);
            if (success) {
              processed.add(postId);
              changed = true;
              this.log(
                `✅ Attached ${postId} to ${reply.parent?.id} at level ${reply.level}`
              );
            }
          }
        }
        ...
        for (const [postId, reply] of this.allReplies.entries()) {
          if (!processed.has(postId)) {
            this.roots.set(postId, reply);
            reply.level = 0;
            this.log(`🌱 Orphan became root: ${postId}`);
          }
        }

        this.log(
          `\n📊 Final tree: ${this.roots.size} roots, ${this.allReplies.size} total posts`
        );
      }
      collectRelevantPosts(startPostId, mode, includeOp = true) {
        ...
        if (!includeOp) {
          ...
          if (opId) {
            relevant.delete(opId);
          }
        }
        this.log(`📦 Collected ${relevant.size} relevant posts (mode: ${mode})`);
        return relevant;
      }

      // Helper method to get the path from OP to a specific post
      getPathFromOpToPost(opId, targetPostId) {
        const path = [];
        let currentId = targetPostId;
        const visited = new Set();

        while (currentId && currentId !== opId && !visited.has(currentId)) {
          visited.add(currentId);
          ...
          const validParents = quotedParents.filter((pid) =>
            this.postGraph.nodes.has(pid)
          );
          if (validParents.length > 0) {
            currentId = validParents.sort((a, b) => parseInt(a) - parseInt(b))[0];
          } else {
            break;
          }
        }
      ...
      getOrganizedThreads() {
        ...
        const sortedRoots = Array.from(this.roots.values()).sort(
          (a, b) => parseInt(a.id) - parseInt(b.id)
        );

        for (const root of sortedRoots) {
          const thread = root.getThread();
          result.push(...thread);
        }
      ...
      printTree() {
        ...
        const printNode = (reply, indent = "") => {
          const marker = reply.isOP
            ? "👑"
            : reply.children.length > 0
            ? "💬"
            : "💭";
          const quotes =
            reply.quotedParents.length > 0
              ? ` (→${reply.quotedParents.join(",")})`
              : "";
          this.log(`${indent}${marker} L${reply.level}: ${reply.id}${quotes}`);
    ...
    class PostGraph {
      ...
      addPost(postId, data, parentIds = []) {
        ...
        if (!this.nodes.has(postId)) {
          node = {
            data: data,
            replies: new Set(),
            parents: new Set(parentIds), // Multiple parents
            id: postId,
          };
        ...
        for (const parentId of node.parents) {
          if (this.nodes.has(parentId)) {
            this.nodes.get(parentId).replies.add(postId);
          }
        }
      ...
      findRoot(postId) {
        ...
        const dfs = (currentId) => {
          ...
          return dfs(sortedParents[0]); // Follow primary parent chain
        };
      ...
      findOP(postId) {
        ...
        const dfs = (currentId) => {
          ...
          if (visited.has(currentId)) {
            this.log(
              `⚠️ Cycle detected in path: ${path.join(" -> ")} -> ${currentId}`
            );
        ...
        const dfs = (currentId) => {
          ...
          if (node.parents.size === 0 || node.data?.op === "1") {
            ...
            if (path.length >= 2) {
              ...
              this.log(
                `ℹ️ No penultimate root - ${currentId} is directly connected to root or is root`
              );
      ...
      getReplyChainFromPost(startPostId, includeOP = true, around = false) {
        this.log(`🎯 Getting organized conversation for ${startPostId}`);
        let mode = around ? "around" : "full";
        const tree = new ConversationTree(this);
        tree.setDebug(this.DEBUG);
        tree.buildFromPostGraph(startPostId, mode, includeOP);
          ...
          if (this.nodes.has(quotedId)) {
            ...
            if (quotedNode) {
              Array.from(quotedNode.replies).forEach((replyId) => {
                ...
                this.log(
                  `↪️ Around: Added reply to quoted parent ${quotedId} -> ${replyId}`
                );
              });
            }
          }
      ...
      collectFullContext(startPostId, toProcess, opId = null) {
      ...
      getConversationAround(postId, includeOP = true, maxDepth = 3) {
        ...
        while (queue.length > 0) {
          ...
          const recentReplies = Array.from(node.replies).sort(
            (a, b) => parseInt(a) - parseInt(b)
          );
      ...
      getReplyChainBFS(rootId, includeOP = true) {
        ...
        while (queue.length > 0) {
          ...
          result.push({
            ...
            data: node.data,
            replies: Array.from(node.replies),
            parents: Array.from(node.parents), // Include all parents
            parentCount: node.parents.size,
          });

          // Add children to queue
          const sortedReplies = Array.from(node.replies).sort(
            (a, b) => parseInt(a) - parseInt(b)
          );
    ...
    async function processNextBacklink() {
      ...
      try {
        ...
        const $postPlaceholder = $(
          '<div class="expanded-post" id="expanded-' + postId + '"></div>'
        );
        $postPlaceholder.css({
          "margin-bottom": "15px",
          padding: "10px",
        ...
        $postPlaceholder.html(
          '<div class="loading">Loading post ' + postId + "...</div>"
        );
        ...
        if ($("#p" + postId).length > 0 || $("#" + postId).length > 0) {
          ...
          const $existingPost =
            $("#p" + postId).length > 0 ? $("#p" + postId) : $("#" + postId);
    ...
    const getPostWrappers = () => {
      return $(".post_wrapper");
    };
    const getPosts = getPostWrappers;
    // Extract link and data-post attribute from the post
    const extractLinkData = (post) => {
    ...
    async function processPostData(data, $placeholder) {
      ...
      try {
        ...
        if (
          !data ||
          (!data.formatted && !data.com && !data.comment && !data.content)
        ) {
        ...
        if (data.formatted) {
          ...
          if (data.name || data.name_processed) {
            ...
            authorElement.textContent =
              data.name_processed || data.name || "Anonymous";
        ...
        $placeholder.html(
          `<div class="error">Error processing post: ${error.message}</div>`
        );
      }
    }
    ...
    const extractThreadInfo = (link) => {
      if (link instanceof HTMLElement) {
        link = $(link);
      }

      if (!link.is("a")) {
        link = link.find(".post_controls > a");
      }
      ...
      if (!postNumber) {
        postNumber =
          link.data("post") ||
          link.closest("article").attr("id")?.replace(/^r/, "");
      }
      ...
      console.log(
        `extractThreadInfo result: threadId=${threadId}, postNumber=${postNumber}`
      );
    ...
    function parseArchiveHTML(html) {
      ...
      const posts = doc.querySelectorAll('article[id^="p"]');
    ...
    function extractPostData(postElement) {
      try {
        ...
        const no = parseInt(postId);

        // Extract timestamp from data-utc attribute
        const timeElement = postElement.querySelector("[data-utc]");
        const time = timeElement
          ? parseInt(timeElement.getAttribute("data-utc"))
          : null;

        // Extract name
        const nameElement = postElement.querySelector(".post_author");
        const name = nameElement ? nameElement.textContent.trim() : "Anonymous";

        // Extract subject
        const subjectElement = postElement.querySelector(".post_title");
        const sub = subjectElement
          ? subjectElement.textContent.trim()
          : undefined;

        // Extract comment
        const commentElement = postElement.querySelector(".text");
        let com = undefined;
        if (commentElement) {
          com = commentElement.innerHTML.trim();
        }

        // Extract image data if present
        let imageData = {};
        const imageElement = postElement.querySelector(".post_file");
        if (imageElement) {
          const imgLink = imageElement.querySelector("a");
          const imgThumb = imageElement.querySelector("img");
          const fileInfoElement = imageElement.querySelector(
            ".post_file_metadata"
          );

          if (imgLink && imgThumb) {
            const fullImageUrl = imgLink.href;
            const filename = imgLink.download || getFilenameFromUrl(fullImageUrl);
            const ext = getFileExtension(filename);

            // Extract dimensions and file size from metadata
            const fileInfo = fileInfoElement ? fileInfoElement.textContent : "";
            const dimensionsMatch = fileInfo.match(/(\d+)x(\d+)/);
            const sizeMatch = fileInfo.match(/([\d.]+)\s*(KB|MB|B)/i);

            imageData = {
              filename: filename,
              ext: ext,
              w: dimensionsMatch ? parseInt(dimensionsMatch[1]) : undefined,
              h: dimensionsMatch ? parseInt(dimensionsMatch[2]) : undefined,
              tn_w: imgThumb.naturalWidth || undefined,
              tn_h: imgThumb.naturalHeight || undefined,
              tim: getTimestampFromUrl(fullImageUrl),
              fsize: sizeMatch
                ? parseFileSize(sizeMatch[1], sizeMatch[2])
                : undefined,
            };
          }
        }

        // Determine if this is OP or reply
        const isOP =
          postElement.classList.contains("op") ||
          postElement.closest(".thread") === postElement;
        ...
        return {
          ...
          sub: sub || undefined,
          com: com || undefined,
          resto: isOP ? 0 : getThreadNumber(postElement),
          ...imageData,
        };
      } catch (e) {
    ...
    function getTimestampFromUrl(url) {
      // Extract timestamp from image URL (usually in format like 1538838346926.jpg)
    ...
    function getThreadNumber(postElement) {
      // Find the thread number from the page context
      const threadElement = postElement.closest("[data-thread-num]");
      if (threadElement) {
        return parseInt(threadElement.getAttribute("data-thread-num"));
      }
    ...
    const fetchReplies = async (link) => {
      ...
      if (!thread || (thread.error && thread.error.length > 0))
        throw new Error("Thread error");

      return extractReplies(postNumber, thread);
    };
    const extractParentIds = (postComment) => {
      if (!postComment) return [];

      const quoteMatches =
        postComment.match(/&gt;&gt;(\d+)/g) || postComment.match(/>(\d+)/g) || [];

      const parentIds = [];
      for (const match of quoteMatches) {
        const quotedNum = match.replace(/&gt;&gt;|>>|>/g, "");
        if (quotedNum && !parentIds.includes(quotedNum)) {
          parentIds.push(quotedNum);
        }
      }
    ...
    const fetchThread = async (board, threadId) => {
      ...
      if (thread && thread.posts) {
        ...
        for (const post of Object.values(thread.posts)) {
          ...
          if (node && node.parents.size === 0 && post.op !== "1") {
            const opId = String(thread.op.num); // First post is usually OP
            if (opId !== postId) {
              node.parents.add(opId);
              postGraph.nodes.get(opId)?.replies.add(postId);
              postGraph.roots.delete(postId);
            }
          }
          if (validParentIds.length > 0) {
            console.log(
              `Added post ${postId} to graph with parents [${validParentIds.join(
                ", "
              )}]`
            );
          } else {
    ...
    async function fetchRepliesWithRetry(aElem, retries = 3, wait = 2000) {
      ...
      for (let attempt = 0; attempt < retries; attempt++) {
        try {
          let replies = await fetchReplies(aElem);
          return replies;
        } catch (error) {
          lastError = error;
          console.error(`Attempt ${attempt + 1} failed: ${error.message}`);

          // Check for rate limiting OR generic thread errors
          if (
            error.status === 403 ||
            error.status === 429 ||
            error.message?.includes("Thread error")
          ) {
            console.warn(`Received error: ${error.message}. Using longer delay.`);
            exponentialWait = Math.min(exponentialWait * 2, 100000); // Cap at 30 seconds
          }

          if (attempt < retries - 1) {
            console.log(
              `Waiting ${exponentialWait}ms before retry ${attempt + 2}...`
            );
            await new Promise((resolve) => setTimeout(resolve, exponentialWait));
          }
        }
      }

      console.error(
        `Failed to fetch replies after ${retries} attempts. Last error: ${
    ...
    function addExpandButtonToPost(post) {
      ...
      if (
        ...
        $postControls.find(".expand-all-btn").length > 0
      ) {
        return;
      }
      ...
      const $expandBtn2 = $(
        '<a href="#" class="btnr parent expand-around-btn">Ard</a>'
      );

      $expandBtn2.click(function (e) {
        e.preventDefault();
        const $btn = $(this);
        expandAllQuotes(post, e.shiftKey, true);
      });
      ...
      const $expandBtn4 = $(
        '<a href="#" class="btnr parent expand-around-btn">NOP</a>'
      );

      $expandBtn4.click(function (e) {
        e.preventDefault();
        const $btn = $(this);
        ...
        if (e.key === "Enter") {
          ...
          if (postCount && !isNaN(postCount)) {
            ...
            randomPosts.each((_, link) => {
              const $link = $(link);
              const postId = $link.text().replace(">>", "");
              if (postId) {
      ...
      $postControls.append($expandBtn4);
...
function updateImageSize(width) {
  ...
  `;
   ...
   const expandAllQuotes = async (
      ...
      // Ensure postWrapper is relative for absolute positioning
      if ($postWrapper.css("position") === "static") {
          $postWrapper.css("position", "relative");
      }
      ...
      $expandedContainer.css({
          ...
          "border-radius": "5px",
          position: "relative"  // backup, but button now refs postWrapper
      });
  ...
  $closeButton.css({
      ...
      top: "10px",         // distance from top of viewport when sticky
      left: "10px",        // distance from left edge
      float: "left",       // helps with positioning
      border: "none",
      "border-radius": "50%",
...
const scrollHandler = () => {
    ...
    if (isContainerVisible) {
      ...
      $closeButton.click(() => {
          let isHidden = $expandedContainer.is(":hidden");
          $expandedContainer.css({
      ...
      $postWrapper.append($closeButton);
      $expandedContainer.append($header);
      $postWrapper.append($expandedContainer);

      try {
          let ps = ids ? ids : [currentPostId];
          const replies = []
          for(let id of ps){
              id = id.replace(/^r/, "");
              const p = all ? postGraph.findRoot(id)
                      : postGraph.findPenultimateRoot(id);
    ...
    async function displayReplyChain(replyChain, $container) {
      const $status = $(
        '<div class="chain-status" style="font-size: 11px; color: #666; margin: 1px 0;"></div>'
      );
      $container.append($status);

      let processed = 0;

      for (const item of replyChain) {
        try {
          ...
          const {
            ...
            quotedParentCount = 0,
            isOP = false,
            hasChildren = false, // New property from ConversationTree
          } = item;

          // Use actual reply count from the structure
          const replyCount =
            hasChildren !== undefined
              ? hasChildren
                ? "??"
                : "0" // Use hasChildren if available
              : replies?.length || 0; // Fallback to replies array
          ...
          // Create level indicator with safety checks
          const $levelHeader =
...
$postContainer.css({
  ...
  "border-bottom": "1px solid #d9d9d9",
          ...
          console.error(`Error displaying post ${item.id || "unknown"}:`, error);
      ...
      try {
        ...
        if (country.length != 0) {
          country.each(function () {
            ...
            title = title.replace(
              "Click here to search for posts with this flag",
              ""
            );
            // Get continent color
    ...
    const processPost = async (post) => {
      ...
      try {
        ...
        let replies = await fetchRepliesWithRetry(aElem, wait * 500, wait);
        ...
        if ($p.hasClass("post_is_op")) {
          ...
          const replies = postGraph.nodes.get(id).replies.size;
          let c = $p.find(".post_type") ?? $p;
          c.append(`Replies: ${replies - 1}`);
        }
  ...
  if (e.key === "+" && e.shiftKey && !e.ctrlKey) { // Shift + = produces "+"
    ...
    saveSettings();
      ...
      if(e.shiftKey && e.key === 'D'){
        e.preventDefault();
        let d = true;
      $(".expanded-posts-container").each((i, con) => {
    const $con = $(con);
        if(i == 0){
          d = $con.is(":visible")
        }
        if(d == $con.is(":visible")){
          $con.toggle();  // toggles display

    // toggle the text visibility opposite to container
    const $text = $con.parent().find(".text");
    $text.toggle(!$con.is(":visible"));
        }
});
      }
    ...
    function addGlobalControls() {
      const $controls = $(
        '<div id="global-expand-controls" style="position: fixed; bottom: 0px; left: 0px; z-index: 9999; background: rgba(0,0,0,0.8); color: white; padding: 0px; border-radius: 5px; font-size: 9px;"></div>'
      );

      const $autoToggle = $(
        '<button id="toggle-auto-expand" style="border: none; border-radius: 3px; cursor: pointer;">Auto-expand: ON</button>'
      );
      const $expandAll = $(
        '<button id="expand-all-posts" style="border: none; border-radius: 3px; cursor: pointer;">Expand All Posts</button>'
      );
    ...
    var pattImageFiletypes = new RegExp(
      "\\.(" + filetypes.IMAGES.join("|") + ")($|(\\?|:)[\\S]+$)",
      "i"
    );
    var pattVideoFiletypes = new RegExp(
      "\\.(" + filetypes.VIDEOS.join("|") + ")($|(\\?|:)[\\S]+$)",
      "i"
    );
    ...
    if (settings.UserSettings.inlineImages.suboptions.customSize.value) {
      imageWidthOP =
        settings.UserSettings.inlineImages.suboptions.customSize.suboptions
          .widthOP.value;
      imageHeightOP =
        settings.UserSettings.inlineImages.suboptions.customSize.suboptions
          .heightOP.value;
      imageWidth =
        settings.UserSettings.inlineImages.suboptions.customSize.suboptions.width
          .value;
      imageHeight =
        settings.UserSettings.inlineImages.suboptions.customSize.suboptions.height
          .value;
    }
    ...
    function generateFavicons() {
      ...
      if (settings.UserSettings.favicon.suboptions.customFavicons.value) {
        switch (
          settings.UserSettings.favicon.suboptions.customFavicons.suboptions.lit
            .value
        ) {
          case "0":
            settings.UserSettings.favicon.suboptions.customFavicons.suboptions.lit.value =
              "https://i.imgur.com/7iTgtjy.png";
            settings.UserSettings.favicon.suboptions.customFavicons.suboptions.alert.value =
              "https://i.imgur.com/QrkQSo0.png";
            break;
          case "1":
            settings.UserSettings.favicon.suboptions.customFavicons.suboptions.lit.value =
              "https://i.imgur.com/AWVjxfw.png";
            settings.UserSettings.favicon.suboptions.customFavicons.suboptions.alert.value =
              "https://i.imgur.com/KXIPcD9.png";
            break;
          case "2":
            settings.UserSettings.favicon.suboptions.customFavicons.suboptions.lit.value =
              "https://i.imgur.com/S7uBSPZ.png";
            settings.UserSettings.favicon.suboptions.customFavicons.suboptions.alert.value =
              "https://i.imgur.com/7IxJvBN.png";
            break;
          case "3":
            settings.UserSettings.favicon.suboptions.customFavicons.suboptions.lit.value =
              "https://i.imgur.com/Rt8dEaq.png";
            settings.UserSettings.favicon.suboptions.customFavicons.suboptions.alert.value =
              "https://i.imgur.com/tvJjpqF.png";
            break;
          case "4":
            settings.UserSettings.favicon.suboptions.customFavicons.suboptions.lit.value =
              "https://i.imgur.com/3bRaVUl.png";
            settings.UserSettings.favicon.suboptions.customFavicons.suboptions.alert.value =
              "https://i.imgur.com/5Bv27Co.png";
            break;
          default:
            break;
        }
        faviconUnlit =
          settings.UserSettings.favicon.suboptions.customFavicons.suboptions.unlit
            .value; // Store unlit favicon
        faviconLit =
          settings.UserSettings.favicon.suboptions.customFavicons.suboptions.lit
            .value; // Store lit favicon
        faviconAlert =
          settings.UserSettings.favicon.suboptions.customFavicons.suboptions.alert
            .value; // Store alert favicon
        faviconNotification =
          settings.UserSettings.favicon.suboptions.customFavicons.suboptions
            .notification.value; // Store notification favicon
        setFavicon();
      } else {
        ...
        nativeFavicon.on("load", function (e) {
          ...
          var faviconData = ctx.getImageData(
            0,
            0,
          ...
          var i, len;
          if (scale - 1) {
            // Only upscale if scale is 2 or more
            faviconNotificationData = new ImageData(
              scale * faviconCanvas.width,
              scale * faviconCanvas.height
            );
            for (i = 0; i < scale; i++) {
              for (var j = 0; j < scale; j++) {
                for (
                  var x = 0, width = faviconCanvas.width, widths = width * 4;
                  x < widths;
                  x += 4
                ) {
                  for (
                    var y = 0,
                      height = faviconCanvas.height,
                      heights = height * 4;
                    y < heights;
                    y += 4
                  ) {
                    ...
                    faviconNotificationData.data[end + 1] =
                      faviconData.data[start + 1];
                    faviconNotificationData.data[end + 2] =
                      faviconData.data[start + 2];
                    faviconNotificationData.data[end + 3] =
                      faviconData.data[start + 3];
                  }
                }
              }
            }
          ...
          for (i = 0, len = faviconData.data.length; i < len; i += 4) {
            var col = new colz.Color(
              faviconData.data[i],
              faviconData.data[i + 1],
              faviconData.data[i + 2],
              faviconData.data[i + 3]
            );
      ...
      add: function (shortcut_combination, callback, opt) {
        ...
        if (!opt) opt = default_options;
          ...
          for (var dfo in default_options) {
            if (
              default_options.hasOwnProperty(dfo) &&
              typeof opt[dfo] == "undefined"
            )
    ...
    function delayedLoad(posts) {
      posts.each(function (i, post) {
        ($(post).hasClass("thread")
          ? $(post).children(".thread_image_box").find("img")
          : $(post).find("img")
        ).each(function (i, image) {
          var $image = $(image);
          $image.data("dontHover", true); // Stop imageHover displaying the thumbnails
          $image.on("click", function (e) {
            // Stop the OP from returning all the sub images and thus duplicating them
      ...
      posts.each(function (i, post) {
        ...
        $imageBoxes.each(function (j, currentImage) {
          ...
          $currentImage.find(">a").each(function (k, imgLink) {
            ...
            if (
              settings.UserSettings.inlineImages.suboptions.processSpoiler
                .value &&
              $currentImage.find(".spoiler_box").length
            ) {
              $(imgLink).html(
                '<div class="spoilerText">Spoiler</div><img class="smallImage spoilerImage">'
              );
              var $image = $currentImage.find("img");
              $image.one("load", function (e) {
                $currentImage
                  .find(".spoilerText")
                  .css({ top: e.target.height / 2 - 6.5 }); // Center spoiler text
              });
            }

            if (/\.webm$/i.test(fullImage)) {
              // Handle post webms
              if (
                settings.UserSettings.inlineImages.suboptions.inlineVideos.value
              ) {
                $currentImage.prepend(
                  '<video width="' +
                    ($(post).hasClass("thread") ? imageWidthOP : imageWidth) +
                    '" name="media" loop muted ' +
                    autoplayVid +
                    '><source src="' +
                    fullImage +
          ...
          $currentImage.find("img.post-image").each(function (m, image) {
            ...
            if (
              settings.UserSettings.inlineImages.suboptions.imageHover &&
    ...
    function filter(posts) {
      posts.each(function (index, currentPost) {
        ...
        if (!/!!UG0p3gRn3T1/.test($currentPost.find(".post_tripcode").html())) {
          if (
            settings.UserSettings.filter.suboptions.recursiveFiltering.value &&
            !$currentPost.hasClass("thread")
          ) {
            // Recursive filter and not OP
            var checkedBacklinks = {};
            $currentPost.find(".text .backlink").each(function (i, backlink) {
              if (
                !checkedBacklinks[backlink.dataset.board + backlink.dataset.post]
              ) {
                // Prevent reprocessing duplicate links
                checkedBacklinks[
                  backlink.dataset.board + backlink.dataset.post
                ] = true;
                var backlinkPost = $("#" + backlink.dataset.post);
                if (backlink.dataset.board === board && backlinkPost.length) {
                  // If linked post is present in thread
                  if (backlinkPost.is(":visible")) {
                    // If linked post is visible
                    if (backlinkPost.hasClass("shitpost")) {
                      // If linked post is a shitpost
                      $currentPost.addClass("shitpost");
                    }
                  } else {
                    // Linked post isn't visible
                    if (backlinkPost.prev().is(":visible")) {
                      // If the hide post stub is visible (and thus the linked post is hidden)
                      togglePost(currentPost.id, "hide");
                    } else {
                      // The linked post has been filtered with mode remove
                      $currentPost.hide();
                    }
                  }
                } else {
                  // Linked post isn't present in thread
                  $.ajax({
                    url: "/_/api/chan/post/",
                    data: {
                      board: backlink.dataset.board,
                      num: backlink.dataset.post,
                    },
                    type: "GET",
                  }).done(function (response) {
                    processPosts(
                      checkFilter(response, false),
                      $currentPost,
                      currentPost
                    );
                    $currentPost
                      .find(".backlink_list .backlink")
                      .each(function (j, replyBacklink) {
                        // Filter replies
          ...
          if ($(currentPost).length) {
            // If after all that the post hasn't been purged
            processPosts(
              checkFilter(currentPost, true),
              $currentPost,
              currentPost
            );
          }
    ...
    function checkFilter(input, inThreadPost) {
      ...
      for (var filterType in settings.FilterSettings) {
        if (settings.FilterSettings.hasOwnProperty(filterType)) {
          var testText = inThreadPost
            ? settings.FilterSettings[filterType].threadPostFunction(input)
            : settings.FilterSettings[filterType].responseObjFunction(input);
          var shortcut = settings.FilterSettings[filterType].value;
          for (var line in shortcut) {
            if (
              shortcut.hasOwnProperty(line) &&
              !shortcut[line].comment &&
              shortcut[line].regex !== undefined
            ) {
              if (
                shortcut[line].boards === undefined ||
                boardPatt.test(shortcut[line].boards)
              ) {
                var regex = new RegExp(
                  shortcut[line].regex.pattern,
                  shortcut[line].regex.flag
                );
    ...
    var embedImages = function (posts) {
      posts.each(function (index, currentArticle) {
        ...
        if (!$currentArticle.data("imgEmbed")) {
          ...
          var imgNum =
            settings.UserSettings.embedImages.suboptions.imgNumMaster.value -
            $currentArticle.find(".thread_image_box").length;
          var isOP = $currentArticle.hasClass("thread");
          (isOP
            ? $currentArticle.children(".text").find("a")
            : $currentArticle.find(".text a")
          ).each(function (index, currentLink) {
            if (imgNum === 0) {
              return false;
            }
            ...
            if (mediaType == "image" || mediaType == "video") {
              imgNum--;
              var filename =
                '<div class="post_file embedded_post_file"><a href="' +
                mediaLink +
                '" class="post_file_filename" rel="tooltip" title="' +
                mediaLink +
                '">' +
                mediaLink.match(/[^\/]*/g)[
                  mediaLink.match(/[^\/]*/g).length - 2
                ] +
                "</a></div>";
              var spoiler = "";
              var $elem = $(
                '<div class="thread_image_box">' + filename + "</div>"
              ).insertBefore(
                isOP
                  ? $currentArticle.children("header")
                  : $currentArticle.find("header")
              );
              ...
              if (mediaType === "image") {
                $elem.append(
                  '<a href="' +
                    mediaLink +
                    '" target="_blank" rel="noreferrer" class="thread_image_link"><img src="' +
                    mediaLink +
                    '" class="lazyload post_image ' +
                    spoiler +
                    'smallImage"></a>'
                );
                ...
                if (!$image.attr("loadEventSet")) {
                  ...
                  $image.on("load", function (e) {
                    ...
                    $(e.target)
                      .closest(".thread_image_box")
                      .append(
                        '<br><span class="post_file_metadata">' +
                ...
                if (
                  settings.UserSettings.embedImages.suboptions.embedVideos.value
                ) {
                  mediaLink = mediaLink.replace(/\.gifv$/g, ".webm"); // Only tested to work with Imgur
                  $elem.append(
                    '<video width="' +
                      imageWidth +
                      '" style="float:left" name="media" loop muted ' +
                      autoplayVid +
                      ' class="' +
                      spoiler +
                      '"><source src="' +
                  ...
                  $elem.find("video")[0].onloadedmetadata = function (e) {
                    $(e.target)
                      .closest(".thread_image_box")
                      .find(".spoilerText")
                      .css({ top: e.target.clientHeight / 2 - 6.5 }); // Center spoiler text
                    $(e.target)
                      .closest(".thread_image_box")
                      .append(
                        '<br><span class="post_file_metadata">' +
              ...
              if (
                settings.UserSettings.embedImages.suboptions.titleYoutubeLinks
                  .value
              ) {
                  ...
                  $.ajax({
                    ...
                    if (response.items.length) {
                      currentLink.innerHTML =
                        "<i>(YouTube)</i> - " + response.items[0].snippet.title;
                    }
                  });
                }
              }
            } else if (
              settings.UserSettings.embedGalleries.value &&
              pattImgGal.exec(currentLink.href) !== null
            ) {
              ...
              if (imgurLinkFragments[3] == "") {
                ...
                imgurLinkFragments[4] = imgurLinkFragments[4].replace(
                  /#[0-9]+/,
                  ""
                ); // Remove the trailing image number
                if (
                  settings.UserSettings.embedGalleries.suboptions.showDetails
                    .value
                ) {
                  $currentArticle
                    .find(".post_wrapper")
                  ...
                  $currentArticle
                    ...
                    .prepend(
                      '<blockquote class="imgur-embed-pub" lang="en" data-id="a/' +
                        imgurLinkFragments[4] +
                        '" data-context="false"><a href="//imgur.com/a/' +
                        imgurLinkFragments[4] +
                        '"></a></blockquote><script async src="//s.imgur.com/min/embed.js" charset="utf-8"></script>'
                    );
                }
                removeLink(currentLink);
              } else if (imgurLinkFragments[3] !== "gallery" && !isOP) {
                // I can't be bothered making this work properly for OPs, so let's just ignore it and pretend it doesn't exist, yes?
                ...
                $.each(individualImages, function (i, imgID) {
                  if (imgNum) {
                    imgNum--;
                    var filename =
                      '<div class="post_file embedded_post_file"><a href="https://i.imgur.com/' +
                      imgID +
                      '.jpg" class="post_file_filename" rel="tooltip" title="https://i.imgur.com/' +
                      imgID +
                      '.jpg">' +
                      imgID +
                      ".jpg</a></div>";
                    var imgHTML =
                      ...
                      imgID +
                      '.jpg" target="_blank" rel="noreferrer" class="thread_image_link"><img src="https://i.imgur.com/' +
                      imgID +
                      '.jpg" class="lazyload post_image smallImage"></a></div>';
                    if (i) {
                      $($currentArticle.find(".thread_image_box")[i - 1]).after(
                        imgHTML
                      );
                    } else {
                ...
                $currentArticle
                  .find(".post_wrapper")
                  .prepend(
                    "<style>.thread_image_box{min-height:" +
                      imageHeight +
                      "px;}.post_wrapper header{min-width:400px;}</style>"
                  );
                $currentArticle
                  .find(".thread_image_box img")
                  .each(function (i, image) {
                    var $image = $(image);
                    if (!$image.attr("loadEventSet")) {
                      $image.attr("loadEventSet", true);
    ...
    function removeLink(currentLink) {
      if ($(currentLink)[0].nextSibling !== null) {
        if ($(currentLink)[0].nextSibling.nodeName == "BR") {
          if (
            $(currentLink)[0].previousSibling === null ||
            $(currentLink)[0].previousSibling.nodeName !== "#text" ||
            $(currentLink)[0].previousSibling.nodeValue == " "
          ) {
    ...
    function imageHover($image) {
      $image.on("mouseenter", function (e) {
        if (
          e.target.id !== "mascot" &&
          !$(e.target).hasClass("bigImage") &&
          !$(e.target).data("dontHover")
        ) {
          $(e.target)
            .clone()
            .removeClass("smallImage spoilerImage")
            .addClass("hoverImage")
            .appendTo("#hoverUI");
        }
      });
      $image.on("mousemove mouseenter", function (e) {
        ...
        if (!$etarget.hasClass("bigImage") && !$etarget.data("dontHover")) {
          var headerBarHeight =
            document.getElementById("headerFixed").offsetHeight - 1; // -1 due to slight offscreen to hide border-top
          var headerBarWidth =
            document.getElementById("headerFixed").offsetWidth - 1; // -1 due to slight offscreen to hide border-right
          ...
          var canFitFullHeight =
            (windowHeight * etarget.naturalWidth) / etarget.naturalHeight <
            visibleWidth - headerBarWidth + 1;
          var $hoverUI = $("#hoverUI");
          var $img = $hoverUI.children("img");
          $img.css({
            "max-height": canFitFullHeight ? windowHeight : visibleHeight,
            "max-width": visibleWidth,
            top: canFitFullHeight
              ? (windowHeight - $img[0].height) * (e.clientY / windowHeight)
              : (visibleHeight - $img[0].height) * (e.clientY / visibleHeight) +
                headerBarHeight,
            left: e.clientX + 50,
          });
        }
      ...
      $video.on("mouseenter", function (e) {
        if (e.target.id !== "mascot" && !$(e.target).hasClass("fullVideo")) {
          ...
          $vid.on("canplaythrough", function () {
            if ($vid.length) {
              // Check if video still exists. This is to prevent the problem where mousing out too soon still triggers the canplay event
              $vid[0].muted = false;
              $vid[0].play();
              $video.on("mousemove", function (e) {
                ...
                var visibleWidth = windowWidth - e.clientX - 50;
                var canFitFullHeight =
                  (windowHeight * e.target.videoWidth) / e.target.videoHeight <
                  visibleWidth - headerBarWidth + 1;
                $vid.css({
                  "max-height": canFitFullHeight ? windowHeight : visibleHeight,
                  "max-width": visibleWidth,
                  top: canFitFullHeight
                    ? (windowHeight - $vid[0].clientHeight) *
                      (e.clientY / windowHeight)
                    : (visibleHeight - $vid[0].clientHeight) *
                        (e.clientY / visibleHeight) +
                      headerBarHeight,
                  left: e.clientX + 50,
                });
              });
    ...
    function relativeTimestamps(posts) {
      posts.find("time").each(function (index, timeElement) {
        if (!$(timeElement).data("relativeTime")) {
          ...
          changeTimestamp(
            timeElement,
            Date.parse($(timeElement).attr("datetime"))
          );
        }
      });
    }
    ...
    function changeTimestamp(timeElement, postTimestamp) {
      ...
      if (diff.y) {
        $(timeElement).html(
          diff.y + " " + years + " and " + diff.d + " " + days + " ago"
        );
        setTimeout(function () {
        ...
        if (diff.d >= 2) {
          ...
          $(timeElement).html(
            diff.d + " " + days + " and " + diff.h + " " + hours + " ago"
          );
          setTimeout(function () {
            changeTimestamp(timeElement, postTimestamp);
          }, 60 * 60 * 1000);
        ...
        if (diff.h >= 2) {
          ...
          $(timeElement).html(
            diff.h + " " + hours + " and " + diff.m + " " + minutes + " ago"
          );
          setTimeout(function () {
            changeTimestamp(timeElement, postTimestamp);
          }, 10 * 60 * 1000);
        ...
        if (diff.m >= 10) {
          ...
          $(timeElement).html(
            diff.m + " " + minutes + " and " + diff.s + " " + seconds + " ago"
          );
          setTimeout(function () {
            changeTimestamp(timeElement, postTimestamp);
          }, 60 * 1000);
    ...
    function seenPosts() {
      ...
      var parsedLastSeenPost = [
        parseInt(lastSeenPost.split("_")[0]),
        parseInt(lastSeenPost.split("_")[1]),
      ];
      $("article").each(function (index, currentArticle) {
        // Add unseen posts to array
        var currentID = [
          parseInt(currentArticle.id.split("_")[0]),
          parseInt(currentArticle.id.split("_")[1]),
        ];
        if (currentID[0] > parsedLastSeenPost[0]) {
          ...
          if (isNaN(currentID[1])) {
            ...
            isNaN(parsedLastSeenPost[1]) ||
            currentID[1] > parsedLastSeenPost[1]
          ) {
            unseenPosts.push(currentArticle.id);
          } else {
      ...
      if (settings.UserSettings.favicon.value) {
        if (unseenPosts.length) {
          ...
          if (windowFocus) {
            ...
            var unseenPostOffset =
              document.getElementById(unseenPosts[0]).offsetTop +
              document.getElementById(unseenPosts[0]).offsetHeight;
            if (unseenPostOffset < viewportBottom) {
              var meanPostHeight =
                (document.getElementById(unseenPosts[unseenPosts.length - 1])
                  .offsetTop -
                  document.getElementById(unseenPosts[0]).offsetTop) /
                unseenPosts.length;
              ...
              var predictedLSP0offset =
                document.getElementById(unseenPosts[predictedLastSeenPostIndex])
                  .offsetTop +
                document.getElementById(unseenPosts[predictedLastSeenPostIndex])
                  .offsetHeight;
              ...
              if (unseenPosts[predictedLastSeenPostIndex + 1] !== undefined) {
                predictedLSP1offset =
                  document.getElementById(
                    unseenPosts[predictedLastSeenPostIndex + 1]
                  ).offsetTop +
                  document.getElementById(
                    unseenPosts[predictedLastSeenPostIndex + 1]
                  ).offsetHeight;
              ...
              while (
                !(
                  predictedLSP0offset <= viewportBottom &&
                  (unseenPosts[predictedLastSeenPostIndex + 1] === undefined ||
                    predictedLSP1offset > viewportBottom)
                )
              ) {
                testedSeeds[predictedLastSeenPostIndex] = true;
                predictedLastSeenPostIndex += Math.floor(
                  (viewportBottom -
                    document.getElementById(
                      unseenPosts[predictedLastSeenPostIndex]
                    ).offsetTop) /
                    meanPostHeight
                );
                ...
                if (testedSeeds[predictedLastSeenPostIndex]) {
                  // Prevent it from getting caught in infinite loops by making sure it never uses the same predictedLSPi twice
          ...
          for (
            var i = predictedLastSeenPostIndex + 1, len = unseenPosts.length;
            i < len;
            i++
          ) {
          ...
          if (predictedLastSeenPostIndex >= 0) {
            ...
            $.each(unseenReplies, function (i, unseenID) {
              // Work from a copy of the array so that removing elements from it doesn't ruin everything
              var currentID = [
                parseInt(unseenID.split("_")[0]),
                parseInt(unseenID.split("_")[1]),
              ];
              if (currentID[0] < parsedLastSeenPost[0]) {
                return true;
              } else if (currentID[0] === parsedLastSeenPost[0]) {
    ...
    function postCounter() {
      if (!Page.is("other,statistics")) {
        var postCount =
          notLoadedPostCount + $(".post_wrapper, div.thread").length;
        var hiddenPostCount =
          $(".stub").length -
          $(".pull-left").children(".btn-toggle-post").filter(":visible").length; // Count total minus those that aren't visible to take account for hiding a whole thread on a board
        var imageCount = $(".thread_image_box").length;
        var limits = settings.UserSettings.postCounter.suboptions.limits.value;
        var countHidden =
          settings.UserSettings.postCounter.suboptions.countHidden.value;
        if (
          countHidden &&
          settings.UserSettings.postCounter.suboptions.countHidden.suboptions
            .hideNullHiddenCounter.value
        ) {
          countHidden = hiddenPostCount;
        }
        var locationHeader =
          settings.UserSettings.postCounter.suboptions.location.value.value ===
          "Header bar";
        ...
        (locationHeader ? $threadStats : $rules_box).html(
          "<" +
            ...
            (limits
              ? "/" +
                settings.UserSettings.postCounter.suboptions.limits.suboptions
                  .posts.value
              : "") +
            (locationHeader ? "" : "<br>") +
            " Images: " +
    ...
    function notifyMe(title, icon, body, timeFade) {
      ...
      if (!Math.floor(Math.random() * 8192)) {
        ...
        icon =
          "http://img.pokemondb.net/sprites/black-white/shiny/" +
          pokemon[ND] +
          ".png";
        timeFade = false;
      }
    ...
    function saveYourPosts() {
      if (yourPosts[board][threadID].length) {
        // If you posted during the thread. Prevents saving of empty arrays
        if (localStorage.yourPosts === undefined) {
          localStorage.yourPosts = JSON.stringify(yourPosts);
        } else {
          localStorage.yourPosts = JSON.stringify(
            $.extend(true, yourPosts, JSON.parse(localStorage.yourPosts))
          );
        }
      }
    }
      ...
      if (localStorage.lastSeenPosts !== undefined) {
        ...
        if (latestLastSeenPosts[board] === undefined) {
          ...
          if (latestLastSeenPosts[board][threadID] === undefined) {
            ...
            if (
              parseInt(latestLastSeenPosts[board][threadID].split("_")[0]) <
              parseInt(lastSeenPosts[board][threadID].split("_")[0])
            ) {
              latestLastSeenPosts[board][threadID] =
                lastSeenPosts[board][threadID];
            } else if (
              parseInt(latestLastSeenPosts[board][threadID].split("_")[0]) ===
              parseInt(lastSeenPosts[board][threadID].split("_")[0])
            ) {
              if (
                (!isNaN(lastSeenPosts[board][threadID].split("_")[1]) &&
                  isNaN(latestLastSeenPosts[board][threadID].split("_")[1])) ||
                parseInt(latestLastSeenPosts[board][threadID].split("_")[1]) <
                  parseInt(lastSeenPosts[board][threadID].split("_")[1])
              ) {
                latestLastSeenPosts[board][threadID] =
                  lastSeenPosts[board][threadID];
              }
    ...
    if (Page.is("thread")) {
      ...
      window.addEventListener("beforeunload", function () {
        // After user leaves the page
        if (settings.UserSettings.labelYourPosts.value) {
          // Save the your posts object
          saveYourPosts();
        }
        if (settings.UserSettings.favicon.value) {
          // Save the last read posts object
          saveLastSeenPosts();
        }
        crosslinkTracker = JSON.parse(localStorage.crosslinkTracker);
    ...
    function notificationSpoiler(postID) {
      ...
      if (settings.UserSettings.notifications.suboptions.spoiler.value) {
        $(temp)
          .find(".spoiler")
          .each(function (i, spoiler) {
            var newSpoilerText = "";
            for (
              var j = 0, spoilerLength = $(spoiler).text().length;
              j < spoilerLength;
              j++
            ) {
      ...
      if (settings.UserSettings.notifications.suboptions.restrict.value) {
        var lineCount =
          settings.UserSettings.notifications.suboptions.restrict.suboptions.lines
            .value;
        var charCount =
          settings.UserSettings.notifications.suboptions.restrict.suboptions
            .characters.value;
        var restrictedText = temp
          .text()
          .trim()
          .substr(0, charCount * lineCount); // Dock three extra chars to replace with ellipses
        ...
        for (var i = 0; i < lineCount; i++) {
          ...
          while (restTextLines[i].length > charCount) {
            // Break up lines that exceed charCount
            if (i < lineCount) {
              restrictedText += restTextLines[i].substr(0, charCount);
              if (!/ /.test(restTextLines[i].substr(0, charCount))) {
                // If there's no space Firefox won't automatically break the chunk onto a new line
                restrictedText += "\n";
              }
    ...
    function labelNewPosts(newPosts, boardView) {
      ...
      for (var boardVal in yourPosts) {
        if (
          yourPosts.hasOwnProperty(boardVal) &&
          crosslinkTracker[board][threadID][boardVal]
        ) {
          ...
          for (var thread in yourPosts[boardVal]) {
            if (yourPosts[boardVal].hasOwnProperty(thread)) {
              for (
                var i = 0, threadLength = yourPosts[boardVal][thread].length;
                i < threadLength;
                i++
              ) {
      ...
      $.each(newPosts, function (i, postID) {
        // For each post returned by update
        var notificationTriggered = false;
        $("#" + postID + " .backlink").each(function (i, link) {
          // For each post content backlink
          var linkBoard = link.dataset.board;
          if (yourPostsLookup[linkBoard] !== undefined) {
            var linkID = link.dataset.post.replace(",", "_");
            if (yourPostsLookup[linkBoard][linkID]) {
              // If the link points to your post
              if (!notificationTriggered && !boardView) {
                if (
                  !settings.UserSettings.filter.value ||
                  !settings.UserSettings.filter.suboptions.filterNotifications
                    .value ||
                  $("#" + postID).filter(":visible").length
                ) {
                  // Filter notifications
                  if (settings.UserSettings.notifications.value) {
                    notifyMe(
                      $("#" + postID + " .post_poster_data")
                        .text()
                        .trim() + " replied to you",
                      faviconNotification,
                      notificationSpoiler(postID),
                      true
                    );
                  }
            ...
            if (yourPostsLookup[linkBoard][postID] && !boardView) {
              // If the post is your own
              var backlink = $(
                "#" + linkID + " .post_backlink [data-post=" + postID + "]"
              );
        ...
        if (document.getElementById(postID) !== null) {
          // Don't add to unseen posts if filter has purged the post
    ...
    function postSubmitEvent() {
      ...
      if (target !== undefined) {
        // Some threads don't allow for ghost posting
        observer.observe(target, config);
      }
    }

    function linkHoverEvent() {
      // Hook into the native internal link hover
      window.MutationObserver =
        window.MutationObserver ||
        window.WebKitMutationObserver ||
        window.MozMutationObserver;
      ...
      var target = $backlink[0],
        observer = new MutationObserver(function () {
          ...
          if (settings.UserSettings.inlineImages.value) {
            ...
            if (
              settings.UserSettings.inlineImages.suboptions.delayedLoad.value ||
              document.URL.includes("b4k")
            ) {
      ...
      if (target !== undefined) {
        // Some boards don't have posts and thus link hovering
        observer.observe(target, config);
      }
    }
    ...
    function mascot(mascotImageLink) {
      ...
      if (settings.UserSettings.mascot.value) {
        if (!$("#mascotBackground").length) {
          ...
          $container_fluid.prepend(
            '<div id="mascotBackground" style="position:fixed; background-color:' +
              $container_fluid.css("background-color") +
              '; z-index:-2; top:0; right:0; bottom:0; left:0;"></div>'
          );
        ...
        if (mascotImageLink !== "") {
          ...
          if (pattVideoFiletypes.test(mascotImageLink)) {
            ...
            if (!$mascotContainer.children().length) {
              $mascotContainer.html(
                '<video id="mascot" style="position:fixed; z-index:-1;" name="media" loop muted autoplay><source src="' +
                  mascotImageLink +
                  '" type="video/webm"></video>'
              );
        ...
        cornerCSS["z-index"] =
          settings.UserSettings.mascot.suboptions.zindex.value;
        ...
        $mascot.on("load", function () {
          // Wait for Mascot to load (otherwise margins won't read size properly)
    ...
    function parseMascotImageValue() {
      var mascotImageLink;
      if (
        isNaN(parseInt(settings.UserSettings.mascot.suboptions.mascotImage.value))
      ) {
        if (
          settings.UserSettings.mascot.suboptions.mascotImage.value ===
            undefined ||
          settings.UserSettings.mascot.suboptions.mascotImage.value === ""
        ) {
          mascotImageLink =
            defaultMascots[Math.floor(Math.random() * defaultMascots.length)]; // If empty set to a random default mascot
        } else {
          mascotImageLink =
            settings.UserSettings.mascot.suboptions.mascotImage.value;
        }
        ...
        mascotImageLink =
          defaultMascots[
            parseInt(settings.UserSettings.mascot.suboptions.mascotImage.value)
          ];
      }
    ...
    function postFlow() {
      if (settings.UserSettings.postFlow.value) {
        ...
        var leftMargin =
          settings.UserSettings.postFlow.suboptions.leftMargin.value;
        var rightMargin =
          settings.UserSettings.postFlow.suboptions.rightMargin.value;
        $("article.thread").each(function (i, threadOP) {
          var $threadOP = $(threadOP);
          if ($threadOP.children(".thread_image_box").length) {
            // Stop posts for intruding in short OPs
            var opTextHeight =
              $threadOP.children(".thread_image_box")[0].offsetHeight +
              10 -
              20 -
              $threadOP.children("header")[0].offsetHeight -
              $threadOP.children(".thread_tools_bottom")[0].offsetHeight;
            $threadOP
              .children(".text")
              .css({ "min-height": opTextHeight + "px" });
          }
        });
        if (settings.UserSettings.mascot.value) {
        ...
        var wordBreak =
          settings.UserSettings.postFlow.suboptions.wordBreak.value.value;
        if (wordBreak === "Auto") {
    ...
    function headerBar() {
      ...
      if (
        settings.UserSettings.headerBar.suboptions.behaviour.value.value ===
        "Collapse to button"
      ) {
        // If in collapse mode
        $headerFixed.removeClass("shortcutHidden"); // Un-hide
        if (
          $headerFixed.find('a[title="Show headerbar"]').filter(":visible").length
        ) {
          if (
            settings.UserSettings.headerBar.suboptions.behaviour.suboptions
              .contractedForm.suboptions.settings.value
          ) {
          ...
          if (
            settings.UserSettings.headerBar.suboptions.behaviour.suboptions
              .contractedForm.suboptions.postCounter.value
          ) {
        ...
        if (!$(".collapseButton").length) {
          // Only add the collapse buttons if they don't already exist
          $boardList.after(
            '<a class="collapseButton" title="Show headerbar" href="javascript:;" style="float:right; display:none; font-family:monospace;">[+]</a>'
          );
          $headerBar.append(
            '<a class="collapseButton" title="Hide headerbar" href="javascript:;" style="font-family:monospace;">[-]</a>'
          );
        }
        $headerBar.find('a[title="Hide headerbar"]').on("click", function () {
        ...
        $headerFixed.find('a[title="Show headerbar"]').on("click", function () {
          // Add click events to the buttons
          $boardList.show();
          $headerBar.children(".threadStats").show();
          $headerBar.children('a[title="SpookyX Settings"]').show();
        ...
        if (
          settings.UserSettings.headerBar.suboptions.behaviour.suboptions.scroll
            .value
        ) {
          // Add a scroll event that collapses it
        ...
        if (
          ...
          // Collapse on pageload
          $headerBar.find('a[title="Hide headerbar"]').trigger("click");
        } else {
          $headerFixed.find('a[title="Show headerbar"]').trigger("click");
        }
      } else {
        // Else remove any collapse stuff
        ...
        if (
          ...
          if (
            ...
            // Add scroll event that hides it
            $(window).on("mousewheel", function (e) {
              if (!$(e.target).closest("#settingsMenu").length) {
                if (e.deltaY > 0) {
                  $headerFixed.removeClass("shortcutHidden");
    ...
    function addFileSelect() {
      if (!$("#file_image").length) {
        $("#reply_elitterae")
          ...
          .parent()
          .append(
            '<div class="input-prepend"><label class="add-on" for="file_image">File</label><input type="file" name="file_image" id="file_image" size="16" multiple="multiple"></div>'
          );
        $(".input-append.pull-left .btn-group [name=reply_gattai]")
          .attr("id", "finalReplySubmit")
          .hide();
        if (!$("#middleReplySubmit").length) {
          var $reply_btngroup = $(".input-append.pull-left .btn-group");
          $reply_btngroup.prepend(
            '<input id="middleReplySubmit" value="Submit" class="btn btn-primary" type="button">'
          );
          $reply_btngroup.append(
            '<span style="float:right;"><input id="urlUploadInput" type="text" style="margin-right: 0;" placeholder="Upload via url"><input id="urlUploadSubmit" class="btn" value="Upload" type="button"></span>'
          );
          ...
          $middleReplySubmit.on("click", function () {
            ...
            if (
              settings.UserSettings.autoHost.value.value !==
              "Don't reupload links"
            ) {
              var fourChanOnly =
                settings.UserSettings.autoHost.value.value ===
              ...
              var linkPatt = fourChanOnly
                ? new RegExp(
                    "https?:\\/\\/(i\\.4cdn\\.org|pbs\\.twimg\\.com)\\/.*\\.(" +
                      concatFileTypes +
                      ")",
                    "ig"
                  )
            ...
            if (fileCount || linksLength) {
              $middleReplySubmit
                .val("Uploading File" + (totalUploads > 1 ? "s" : ""))
                ...
                $.each(links, function (i, link) {
                  if (!/i\.imgur\.com/i.test(link)) {
                    $.ajax({
                      ...
                      .done(function (response) {
                        document.getElementById("reply_chennodiscursus").value =
                          document
                            .getElementById("reply_chennodiscursus")
                            .value.replace(
                              link,
                      ...
                      .fail(function (e) {
                        notifyMe(
                          ...
                          "http://i.imgur.com/qEpGpTc.png",
                          "The link was: " +
                            link +
                            "\n" +
                            JSON.parse(e.responseText).data.error,
                          false
                        );
                      });
                  }
              ...
              if (fileCount) {
                $.each(
                  document.getElementById("file_image").files,
                  function (i, file) {
                    var reader = new FileReader();

                    if (false && /image/.test(file.type)) {
                      // Disable Imgur usage, don't remove code in case it needs to be restored
    ...
    function updateExportLink() {
      // Define the export settings link with the latest version of the settings
      var $settingsExport = $("#settingsExport");
      $settingsExport.attr(
        "download",
        "SpookyX v." + GM_info.script.version + "-" + Date.now() + ".json"
      );
      $settingsExport.attr(
        "href",
        "data:" +
    ...
    function revealSpoilers() {
      if (settings.UserSettings.revealSpoilers.value) {
        $("#SpookyX-css-hovered-spoilers").html(
          ".spoiler {color:" + hoveredTextColourPicker + "!important;}"
        );
      } else {
        $("#SpookyX-css-hovered-spoilers").html("");
      }
    ...
    $(document).ready(function () {
      ...
      var $head = $("head");
      $body.append(
        '<div id="postBackgroundColourPicker" class="thread_form_wrap" style="display:none;"></div>'
      ); // Create an element to get the post colour from
      var $postBackgroundColourPicker = $("#postBackgroundColourPicker");
      var postBackgroundColourPicker =
        $postBackgroundColourPicker.css("background-color"); // Set the colour
      $postBackgroundColourPicker.remove(); // Delete the element afterwards

      var mockHoverFailed = false;
      try {
        // Firefox can fail with a security error (something to do with stylesheets from multiple domains)
      ...
      var $hoveredTextColourPicker = $(
        '<div class="spoiler mock-hover" style="position: fixed; top: -1000px;">hoveredTextColourPicker</div>'
      ); // Create an element to get the hovered text colour from
      ...
      if (mockHoverFailed) {
        ...
        hoveredTextColourPicker = hoveredTextColourPicker.replace(
          /([0-9]+)[^0-9]+([0-9]+)[^0-9]+([0-9]+)/,
          function ($0, $1, $2, $3) {
      ...
      $head.after(
        '<style type="text/css" id="SpookyX-css-hovered-spoilers"></style>'
      ); // Add style element that controls that one thing
      $("#SpookyX-css").append(
        ".imgur-embed-iframe-pub{float: left; margin: 10px 10px 0 0!important;}.post_wrapper .pull-left, article.backlink_container > div#backlink .pull-left{display:none;}" +
          ...
          ".hoverImage{position:fixed;float:none!important;}.bigImage{opacity: 1!important; max-width:100%;}.smallImage{max-width:" +
          imageWidth +
          "px; max-height:" +
          imageHeight +
          "px}" +
          ".smallImage.thread_image{max-width:" +
          imageWidthOP +
          "px; max-height:" +
          imageHeightOP +
          "px}.spoilerImage{opacity: 0.1}.spoilerText{position: relative; height: 0px; font-size: 19px; top: 47px;}" +
          ...
          ".shitpost{opacity: 0.3}.embedded_post_file{margin: 0!important; max-width: " +
          imageWidth +
          ...
          ".suboption-list > div:last-of-type {background-color:" +
          postBackgroundColourPicker +
          ";}#settingsMenu input{margin: 3px 3px 3px 4px; padding-top:1px; padding-bottom:0; padding-right:0;}" +
          ...
          '#settingsMenu input[type="number"]{height:16px; line-height:0; width:44px;}.last{background-color:' +
          postBackgroundColourPicker +
          ";}#settingsMenu code{padding: 2px 4px; background-color: #f7f7f9!important; " +
      ...
      if (settings.UserSettings.favicon.value) {
        ...
        generateFavicons();
        $("#reply fieldset .progress").after(
          '<canvas id="myCanvas" width="64" height="64" style="float:left; display:none; position: relative; top: -10px; left: -10px;"></canvas>'
        );
      }
      ...
      if ($letters.length) {
        $letters.html(
          '<span class="boardList">' +
            $letters.html() +
            '</span><span class="headerBar"><div class="threadStats"></div><a title="SpookyX Settings" href="javascript:;" style="margin-right:10px;">Settings</a></span>'
        );
        ...
        // Insert settings link when on board index
        $(".container-fluid").append(
          '<div class="headerBar" style="position: fixed; right: 0; top: 0;"><a title="SpookyX Settings" href="javascript:;">Settings</a></div>'
        );
      }
      $body.append(
        '<div id="settingsMenu" class="thread_form_wrap" style="display: none;"><input type="file" id="fileInput" style="display:none;"><div id="settingsHeader"><div class="sections-list"><a href="javascript:;" class="active">Main</a> | <a href="javascript:;">Filter</a></div><div class="credits"><a id="settingsExport" title="Export" href="javascript:;">Export</a> | <a id="settingsImport" title="Import" href="javascript:;">Import</a> | <a title="Reset Settings" href="javascript:;">Reset Settings</a> | <a target="_blank" href="https://github.com/Fiddlekins/SpookyX" style="text-decoration: underline;">SpookyX</a> | <a target="_blank" href="https://github.com/Fiddlekins/SpookyX/blob/master/CHANGELOG.md" style="text-decoration: underline;">v.' +
          GM_info.script.version +
          '</a> | <a target="_blank" href="https://github.com/Fiddlekins/SpookyX/issues" style="text-decoration: underline;">Issues</a> | <a title="Close" href="javascript:;">Close</a></div></div><div id="menuSeparator"></div><div id="settingsContent"></div></div>'
      );
      ...
      if (Page.is("thread")) {
        // If in a thread
        $($(".navbar .nav")[1]).append(
          '<li><a href="//boards.4chan.org/' +
            board +
            "/thread/" +
            threadID +
            '">View thread on 4chan</a></li>'
        ); // Add view thread on 4chan link
      }
      $('.headerBar > a[title="SpookyX Settings"], a[title=Close]').on(
        "click",
        function () {
      ...
      if (window.scrollY > 42) {
        // Show headerbar on pageload if necessary
        $("#headerFixed").show();
      }
      $(".sections-list").on("click", function (e) {
        // Main settings tabs change on click
      ...
      $("#settingsContent").on("click", function (e) {
        // Filter subtabs change on click
      ...
      $("#settingsMenu .credits > a").on("click", function (e) {
        // Set up the import/export/reset links
        if (e.target.title === "Import") {
          // Import settings
      ...
      $("#fileInput").on("change", function () {
        // When the undisplayed file input element changes import settings
      ...
      $("#settingsContent").on("change", function (e) {
        if ($(e.target).hasClass("filterTextarea")) {
                ...
                if (fragment !== "") {
                  if (!i) {
                    ...
                    if (/[gim]/.test(regex.substring(regex.length - 1))) {
                      ...
                      lineStore.regex = {
                        pattern: regex.substring(1, regex.length - 1),
                        flag: "",
                      };
                    }
                  } else {
          ...
          if (e.target.type === "checkbox") {
            ...
            $(e.target)
              .closest("div:not(.settingFlexContainer)")
              .children(".suboption-list")
              .toggle(); // Make parent checkboxes collapse the suboptions if they're unticked
            $(e.target)
              .closest(".settingFlexContainer")
              .children(".settingsJoinLine")
              .toggle();
          } else {
          ...
          if (e.target.nodeName == "SELECT") {
            ...
            for (var suboption in settingPath.suboptions) {
              if (
                settingPath.suboptions.hasOwnProperty(suboption) &&
                settingPath.suboptions[suboption].if !== undefined
              ) {
                var ifMet = false;
                ...
                if (ifMet) {
                  $(e.target)
                    .closest("div:not(.settingFlexContainer)")
                    .find(".suboption-list [key=" + suboption + "]")
                    .closest("div:not(.settingFlexContainer)")
                    .show();
                } else {
                  $(e.target)
                ...
                $(e.target)
                  .closest("div:not(.settingFlexContainer)")
                  .find(".suboption-list > .last")
                  .removeClass("last");
                $(e.target)
                  .closest("div:not(.settingFlexContainer)")
                  .find(".suboption-list > :visible:last")
                  .addClass("last");
                if (
                  $(e.target)
                    .closest("div:not(.settingFlexContainer)")
                    .find(".suboption-list > :visible").length > 1
                ) {
                  $(e.target)
                    .closest(".settingFlexContainer")
                    .children(".settingsJoinLine")
          ...
          if (elementPath.substr(0, 6) === "mascot") {
            // Live update changes in mascot settings
        ...
        if (
          e.target.name === "Custom Favicons" ||
          (e.target.name === "Favicon" && e.target.checked)
        ) {
          generateFavicons();
        }
        saveSettings(); // Save the settings
      ...
      if (settings.UserSettings.postCounter.suboptions.countUnloaded.value) {
        if (Page.is("thread")) {
          // Count the posts that aren't loaded (eg. in last/50 mode)
          $.ajax({
            url: "/_/api/chan/thread/",
            method: "GET",
            data: { board: board, num: threadID, inThread: true },
          }).done(function (response) {
            var firstLoadedPostID = $("article.post").length
              ? $("article.post")[0].id
              : null;
      ...
      if (!Page.is("search,other,statistics")) {
        ...
        $(document).ajaxComplete(function (event, request, ajaxSettings) {
          if (
            (!/inThread=true/i.test(ajaxSettings.url) &&
              /api\/chan\/thread\/\?/i.test(ajaxSettings.url)) ||
            (ajaxSettings.type === "POST" && /\/submit\//i.test(ajaxSettings.url))
          ) {
            ...
            if (request.responseText !== "") {
              try {
                if (request.responseText.charAt(0) === "<") {
                  ...
                  response = {
                    error:
                      "SpookyX encountered an error when parsing the response. The connection" +
                ...
                response = {
                  error:
                    "SpookyX encountered an error when parsing the response.",
                };
                console.log("The following is the request responseText:");
                console.log(request.responseText);
                alert(
                  "A nasty error has occurred. Please take a screenshot of your console and give it to " +
            ...
            if (Page.is("thread")) {
              if (ajaxSettings.type === "POST") {
                if (response.error === undefined) {
                  if (response.captcha) {
                    // If you are required to fill a captcha before posting
                    //console.log(response);
                  } else {
                    for (postID in response[threadID].posts) {
                      if (
                        response[threadID].posts.hasOwnProperty(postID) &&
                        response[threadID].posts[postID].comment.replace(
                          /[\r\n]/g,
                          ""
                        ) == lastSubmittedContent.replace(/[\r\n]/g, "")
                      ) {
                    ...
                    for (var boardVal in crosslinkTracker) {
                      if (crosslinkTracker.hasOwnProperty(boardVal)) {
                        for (var threadVal in crosslinkTracker[boardVal]) {
                          if (
                            crosslinkTracker[boardVal].hasOwnProperty(threadVal)
                          ) {
                    ...
                    localStorage.crosslinkTracker =
                      JSON.stringify(crosslinkTracker);
                    saveYourPosts();
                  ...
                  if (settings.UserSettings.notifications.value) {
                    notifyMe(
                      "An error occurred whilst posting",
                      faviconNotification,
                      response.error,
                      false
                    );
            ...
            for (var key in response) {
              if (
                ...
                response[key] !== null &&
                response[key].posts !== undefined
              ) {
                if (
                  Page.is("board") &&
                  settings.UserSettings.labelYourPosts.value
                ) {
                  // Handle (You) deignation for expanding threads on board view
                  labelNewPosts(Object.keys(response[key].posts), true);
                }
                for (postID in response[key].posts) {
                  if (
                    response[key].posts.hasOwnProperty(postID) &&
                    document.getElementById(postID) !== null
                  ) {
                    // Don't process post if filter has purged it
                    $newPost = $("#" + postID);
                    if (settings.UserSettings.inlineImages.value) {
                      ...
                      if (
                        settings.UserSettings.inlineImages.suboptions.delayedLoad
                          .value ||
                        document.URL.includes("b4k")
                      ) {
                        delayedLoad($newPost);
                    ...
                    if (settings.UserSettings.postQuote.value) {
                      $newPost
                        .find(".post_data > [data-function=quote]")
                        .removeAttr("data-function")
                        .addClass("postQuote");
                    } // Change the quote function
                    if (settings.UserSettings.hidePosts.value) {
                      $newPost.children(".pull-left").removeClass("stub");
                      if (
                        settings.UserSettings.hidePosts.suboptions.recursiveHiding
                          .value
                      ) {
                        $newPost
                          .find(".post_backlink")
                          .attr("id", "p_b" + postID);
                        if (
                          settings.UserSettings.hidePosts.suboptions
                            .recursiveHiding.suboptions.hideNewPosts.value
                        ) {
                          var checkedBacklinks = {};
                          $newPost
                            .find(".text .backlink")
                            .each(function (i, backlink) {
                              if (
                                !checkedBacklinks[
                                  backlink.dataset.board + backlink.dataset.post
                                ]
      ...
      if (settings.UserSettings.inlineImages.value) {
        ...
        if (!Page.is("statistics")) {
          // Stop this interfering with the images it displays
          $("#main img").each(function (i, image) {
            var $image = $(image);
            $image.addClass("smallImage");
            if (Page.is("search,quests")) {
              if ($image.attr("width") >= 249 || $image.attr("height") >= 249) {
                // Assuming OP images are 250px limited across all archives might be false
      ...
      if (settings.UserSettings.labelYourPosts.value) {
        ...
        if (Page.is("search,board,quests,gallery")) {
          if (board === "_") {
            // Handle finding the board per post for all-board searches
            $("article.post").each(function (i, post) {
              var postBoard = $(post)
                .find(".post_show_board")
                .html()
                .replace(/\//g, "");
            ...
            // Handle the lack of threadID for board indexes and single-board searches
          ...
          // Handle regular threads by iterating over the yourPosts values for that specific thread (better performance than per each post parsing)
          $.each(yourPosts[board][threadID], function (i, v) {
            // Parse all backlinks present on pageload
            $("#" + v)
              .find(".post_author")
              .after("<span> (You)</span>");
          });
        ...
        $(".backlink").each(function (i, backlink) {
          if (
            yourPostsLookup[backlink.dataset.board] !== undefined &&
            yourPostsLookup[backlink.dataset.board][
              backlink.dataset.post.replace(",", "_")
            ]
          ) {
        ...
        if (settings.UserSettings.hidePosts.suboptions.recursiveHiding.value) {
          ...
          $("article.post")
            ...
            .each(function (i, post) {
              // Recursively hide pre-hidden posts
      ...
      $("#main").on("click", function (e) {
        // Detect clicks on page content
        clickHandler(e);
      });
      function clickHandler(e, called = false) {
        if (
          settings.UserSettings.inlineReplies.value &&
          $(e.target).hasClass("backlink")
        ) {
          if (!e.originalEvent?.ctrlKey && e.which == 1) {
            e.preventDefault();
            var etarget = e.target;
            ...
            if (etarget.parentNode.className == "post_backlink") {
              if ($etarget.hasClass("inlined")) {
                ...
                $(".sub" + rootPostID).each(function (index, currentPost) {
                  $("#" + currentPost.id.substr(1) + ".forwarded").removeClass(
                    "forwarded"
                  );
                });
                ...
                $("#" + postID)
                  .addClass("forwarded")
                  .clone()
                  .removeClass("forwarded base post")
                  .attr("id", "r" + postID)
                  .show()
                  .appendTo($("#i" + postID + ".sub" + rootPostID));
                $("#" + rootPostID + ".base .inline").each(function (
                  index,
                  currentPost
                ) {
                ...
                $("#i" + postID + " .post_wrapper").addClass(
                  "post_wrapperInline"
                );
                if (settings.UserSettings.inlineImages.value) {
          ...
          settings.UserSettings.postQuote.value &&
          e.target.className === "postQuote"
        ) {
          // Better post quoting
          if (!e.originalEvent.ctrlKey && e.which == 1) {
            e.preventDefault();
            ...
            if (input.selectionStart !== undefined) {
              ...
              document.getElementById("reply_chennodiscursus").value =
                originalText.replace(originalText, newText);
            }
          }
        } else if (
          settings.UserSettings.inlineImages.value &&
          ...
          if ($video.hasClass("fullVideo")) {
            if (
              !settings.UserSettings.inlineImages.suboptions.inlineVideos
                .suboptions.firefoxCompatibility.value ||
              e.pageY < video.offsetTop + video.videoHeight - 28
            ) {
              // Firefox blows. FF controls are 28px tall at this point
              video.pause();
              video.muted = true;
              $video.attr(
                "width",
                $video.closest("article").hasClass("thread")
                  ? imageWidthOP
                  : imageWidth
              );
          ...
          e.target.className === "btn-toggle-post" ||
          e.target.parentNode.className === "btn-toggle-post"
        ) {
          // If a hide post button is clicked
          var button =
            e.target.className === "btn-toggle-post"
              ? e.target
              : e.target.parentNode;
          if (settings.UserSettings.hidePosts.suboptions.recursiveHiding.value) {
            // If recursive toggling is enabled
            if (button.attributes["data-function"].value === "showPost") {
              recursiveToggle(
                $(
                  "article.doc_id_" + button.attributes["data-doc-id"].value
                ).attr("id"),
                "show"
              );
          ...
          var waitForNativeHide = setInterval(function () {
            // Calling postCounter immediately does so before the native site toggles everything
      ...
      exclam.src =
        settings.UserSettings.favicon.suboptions.customFavicons.suboptions.alertOverlay.value;
      ...
      function galleryUpdate() {
        if ($("#gallery").length) {
          ...
          if ($(imgList[imgIndex]).find("img").length) {
            $("#gallery").html(
              '<img style="max-width:90%; max-height:90%;" src="' +
                $(imgList[imgIndex]).find("img")[0].src +
                '">'
            );
          } else if ($(imgList[imgIndex]).find("video").length) {
            $("#gallery").html(
              '<video style="float:left; max-width:90%; max-height:90%;" name="media" loop muted controls ' +
                autoplayVid +
                '><source src="' +
                $(imgList[imgIndex]).find("video")[0].currentSrc +
                '" type="video/webm"></video>'
            );
          } else {
          ...
          $(document).scrollTop(
            $(imgList[imgIndex]).find("img, video").offset().top - 26
          );
        }
      }
      ...
      function populateSettingsMenu() {
        if ($("#settingsMenu").is(":visible")) {
          ...
          var settingsHTML =
            '<div id="Main">' +
            generateSubOptionHTML(settings.UserSettings, "") +
            "</div>";
          ...
          $("#settingsContent select, #settingsContent input").each(function (
            i,
            el
          ) {
            if (el.type !== "checkbox") {
              // Add the top margins for non-checkboxes to align description with name
              $(el).parent().next().addClass("selectDescription");
            }
            if (el.nodeName === "SELECT") {
              // Hide the settings join line for select options that start with one or less visible suboptions
              var visibleSubopsCount = 0;
              $(el)
                .closest("div:not(.settingFlexContainer)")
                .children(".suboption-list")
                .children()
                .each(function (i, subop) {
      ...
      function generateFilterHTML() {
        ...
        var type;
        settingsHTML +=
          '<div class="filters-list"><a href="javascript:;" class="active" name="guide">Guide</a>';
        ...
        for (type in settings.FilterSettings) {
          if (settings.FilterSettings.hasOwnProperty(type)) {
            ...
            $.each(settings.FilterSettings[type].value, function (i, line) {
              ...
              if (!line.comment) {
                if (
                  line.regex !== undefined &&
                  line.regex.pattern !== undefined
                ) {
                  settingsHTML +=
                    "/" + line.regex.pattern + "/" + line.regex.flag + ";";
                  for (var prop in line) {
                    if (
                      line.hasOwnProperty(prop) &&
                      prop !== "comment" &&
                      prop !== "regex"
                    ) {
        ...
        $.each(input, function (key, value) {
          if (value.name !== undefined) {
            ...
            if (value.if !== undefined) {
              var parentPath = objpath(
                settings.UserSettings,
                path.substring(0, path.length - ".suboptions.".length)
              );
            ...
            if (
              value.suboptions !== undefined &&
              Object.keys(value.suboptions).length > 1
            ) {
        ...
        function handleClicks(aElems) {
          ...
          while (queue.length > 0) {
            ...
            if (i && !open.includes(i)) {
              ...
              let newSelector = inverse
                ? `.text > #i${i} .text .backlink`
                : ".post_backlink > a.backlink";
                  ...
                  try {
                    ...
                    if (opElem) {
                      ...
                      setTimeout(() => {
                        ...
                        const $createdPost =
                          $inlineOpElement.find("article.post");
                        if ($createdPost.length > 0) {
                          processPost($createdPost[0]).catch(console.error);
                        }
                      }, 100);
